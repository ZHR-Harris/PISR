...
def adjust_learning_rate(config, epoch):
    lr = config.optimizer.params.lr * \
        (0.5 ** (epoch // config.scheduler.params.step_size))
    return lr
...
def train_single_epoch(config, student_model, teacher_model, dataloader, criterion,
    ...
    for i, (LR_patch, HR_patch, filepath) in tbar:
        ...
        student_pred_dict = student_model.forward(
            LR=LR_patch, teacher_pred_dict=teacher_pred_dict)
        loss = criterion['train'](
            teacher_pred_dict, student_pred_dict, HR_patch)
...
def evaluate_single_epoch(config, student_model, teacher_model, dataloader,
    ...
    with torch.no_grad():
        ...
        for i, (LR_img, HR_img, filepath) in tbar:
            ...
            student_pred_dict = student_model.forward(
                LR=LR_img, teacher_pred_dict=teacher_pred_dict)
...
def run(config):
    ...
    print('best psnr : %.3f, best epoch: %d' %
          (result['best_psnr'], result['best_epoch']))
